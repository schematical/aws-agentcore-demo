# Execution role for the harness resource in this folder's main.tf (Stage 5).
...
resource "aws_iam_role_policy" "agent_execution" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      {
        Sid      = "FetchTerraformDiagramSkill"
        Effect   = "Allow"
        Action   = ["s3:GetObject"]
        Resource = "${aws_s3_bucket.diagrams.arn}/skills/terraform-diagram/*"
      },
      {
        Sid      = "ListTerraformDiagramSkillPrefix"
        Effect   = "Allow"
        Action   = ["s3:ListBucket"]
        Resource = aws_s3_bucket.diagrams.arn
        Condition = {
          StringLike = {
            "s3:prefix" = "skills/terraform-diagram/*"
          }
        }
      },
    ]
