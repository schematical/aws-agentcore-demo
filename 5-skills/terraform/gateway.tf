# Unchanged from 4-mcp-gateway/terraform - Stage 5 keeps the Gateway active,
# it just adds the Code Interpreter tool and a skill block on top (see main.tf).
...
resource "aws_bedrockagentcore_gateway_target" "search_knowledge_base" {
  # Waits on the gateway role's trust-policy propagation delay - see
  # time_sleep.gateway_iam_propagation in iam.tf.
